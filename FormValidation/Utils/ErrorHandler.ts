...
export class ValidationErrorHandler {
   ...
   */
  public async handleRemoteError(
    error: Error,
    endpoint?: string,
    retryCount = 0
    ...
    if (retryCount < maxRetries) {
      ...
      return {
        ...
        error: this.createError(
          ...
          { endpoint, retryCount, maxRetries }
        )
      };
    }
    ...
    const validationError = this.createError(
      ...
      'Remote validation failed after maximum retry attempts',
      { endpoint, retryCount, maxRetries, originalError: error.message },
      error
    );
  ...
  public handleParserError(
    ...
    const validationError = this.createError(
      'PARSER_ERROR',
      'SCHEMA_PARSE_FAILED',
      'Failed to parse validation schema',
      { schema, fieldName, originalError: error.message },
      error
    );
  ...
  public handleRegistryError(
    ...
    const validationError = this.createError(
      'REGISTRY_ERROR',
      'VALIDATOR_NOT_FOUND',
      `Validator '${validatorType}' not found in registry`,
      { validatorType, originalError: error.message },
      error
    );
  ...
  public getErrorStats(): {
    totalErrors: number;
    errorTypes: Record<ValidationErrorType, number>;
   ...
   */
  public exportErrorLog(): ValidationError[] {
   ...
   */
  private getLogMethod(errorType: ValidationErrorType): (...args: any[]) => void {
    switch (errorType) {
   ...
   */
  private getFallbackResult(error: ValidationError): ExtendedValidationResult {
    switch (error.type) {
      case 'VALIDATION_FAILED':
        return {
          valid: false,
   ...
   */
  private getBasicFallbackValidation(fieldName?: string): ExtendedValidationResult {
    return {
...
export function withErrorHandling<T extends any[], R>(
  ...
  descriptor.value = async function(...args: T): Promise<R> {
    try {
      ...
      const errorResult = validationErrorHandler.handleValidationError(
        error as Error,
        undefined,
        propertyKey
      );
      ...
      if (errorResult.fallbackResult) {
        return errorResult.fallbackResult as R;
      }
